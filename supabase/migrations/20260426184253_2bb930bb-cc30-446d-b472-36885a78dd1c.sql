CREATE OR REPLACE FUNCTION public.create_followup_case(_parent_case_id uuid, _case_type text, _with_new_work boolean DEFAULT false, _charge_mode text DEFAULT 'free'::text, _custom_price numeric DEFAULT NULL::numeric, _notes text DEFAULT NULL::text, _due_date date DEFAULT NULL::date)
  RETURNS uuid
  LANGUAGE plpgsql
  SECURITY DEFINER
...
BEGIN
  ...
  -- Always start follow-up cases (remake/repair) from the workflow's START stage
  -- (e.g. "تم استلام الحالة" / lab intake), not from the parent's current stage.
  ...
  -- Fallback: lowest order_index stage if no explicit start flag
  IF v_start_stage IS NULL THEN
    SELECT id INTO v_start_stage
    FROM public.workflow_stages
  ...
  INSERT INTO public.cases (
    lab_id, case_number, doctor_id, patient_id, work_type_id, workflow_id,
    current_stage_id, shade, tooth_numbers, units, notes, price, status,
    date_received, due_date, stage_entered_at, created_by, parent_case_id,
    case_type, charge_mode
  ) VALUES (
    v_parent.lab_id, v_new_number, v_parent.doctor_id, v_parent.patient_id,
    v_parent.work_type_id, v_workflow_id, v_target_stage, v_parent.shade,
    v_parent.tooth_numbers, COALESCE(v_parent.units, 1),
    COALESCE(_notes, CASE WHEN _case_type = 'remake' THEN 'إعادة للحالة ' ELSE 'تصليح للحالة ' END || v_parent.case_number),
    v_price, 'active'::public.case_status, CURRENT_DATE, _due_date, now(),
    auth.uid(), _parent_case_id, _case_type, _charge_mode
  ) RETURNING id INTO v_new_id;

  IF v_parent_has_items THEN
    INSERT INTO public.case_items (
      case_id, lab_id, notes, position, shade, tooth_numbers,
      total_price, unit_price, units, work_type_id
    )
    SELECT
      v_new_id, v_parent.lab_id, ci.notes, ci.position, ci.shade, ci.tooth_numbers,
      CASE WHEN _charge_mode = 'free' THEN 0 ELSE ci.total_price END,
      CASE WHEN _charge_mode = 'free' THEN 0 ELSE ci.unit_price END,
      COALESCE(ci.units, 1), ci.work_type_id
  ...
  ELSIF v_parent.work_type_id IS NOT NULL THEN
    INSERT INTO public.case_items (
      ...
      v_new_id, v_parent.lab_id, v_parent.notes, 0, v_parent.shade, v_parent.tooth_numbers,
      ...
      GREATEST(COALESCE(v_parent.units, 1), 1), v_parent.work_type_id
    );
  END IF;
  ...
  VALUES (
    v_parent.lab_id, auth.uid(), 'case', v_new_id,

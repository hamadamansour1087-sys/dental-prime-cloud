...
DROP FUNCTION IF EXISTS public.create_followup_case(uuid,text,boolean,text,numeric,text,date);
...
BEGIN
  ...
  -- Copy case attachments (photos, scans, STL files, etc.)
  INSERT INTO public.case_attachments (
    case_id, lab_id, storage_path, file_name, file_size, mime_type, kind, uploaded_by
  )
  SELECT
    v_new_id, v_parent.lab_id, ca.storage_path, ca.file_name,
    ca.file_size, ca.mime_type, ca.kind, ca.uploaded_by
  FROM public.case_attachments ca
  WHERE ca.case_id = _parent_case_id;
